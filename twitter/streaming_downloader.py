...
class FileListener(StreamListener):
        ...
        def on_data(self, data):
                ...
                if self.current_file == None or time.time() - self.restart_time > self.file_start_time or self.file_start_date.day != current_time.day:
                        self.startFile()
                        self.file_start_date = datetime.datetime.now()
                if data.startswith('{'):
                        data = data.rstrip()
...
if __name__ == '__main__':
        options = [['consumer_key=', 'The consumer key.', True, None],['consumer_secret=', 'The consumer key secret.', True, None],
                   ['access_token=', 'The access token.', True, None],['access_token_secret=', 'The access token secret.', True, None],
                   ['stream_type=', 'The type of stream to run: sample, location, keyword.', True, None],['output_directory=', 'Where to save output files.', True, None],
                   ['stream_filename=', 'The name of the file containing parameters for this stream. Required for location and keyword.', False, ''],
                   ['pid_file=', 'Save the pid of this job to the given file.', False, None],['log_filename=', 'The log file.', True, None]]
        ...
        if stream_type.lower() == 'location' or stream_type.lower() == 'keyword':
                ...
                stream_args=[-81.73333,-4.2166,-66.85,13.3833]
                #stream_args =['galletas noel', 'galletasnoel','saltin noel','saltinnoel', 'ducales','tosh','galletas festival',
                             # 'galletasfestival','pozuelo','dux','chiky','chocolatesjet','chocolates jet','chocolate corona',
                             # 'chocolatecorona','nucita', 'chocolatesjumbo','chocolates jumbo','chocolisto','chocolyne',
                              #'fochis','chinchin','cafesellorojo','cafe sello rojo','colcafe','cafe la bastilla','cafelabastilla',
                              #'cafematiz','cafe matiz']
